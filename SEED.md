...
- Already-obtained creds (from a real prior phone-bind) MAY be placed at install time via `--from-env`, which reads `PLOW_CHAT_TOKEN` and `PLOW_CHAT_CHAT_UID` from the environment (plus optional `PLOW_CHAT_BASE_URL`, default `https://api.plow.co`, and `PLOW_CHAT_HOME_CHANNEL`, default = chat uid), writes them to the resolved target, and skips the phone-bind dance entirely. This is a real, supported operator path — distinct from `--test-mode` — used to place creds the operator obtained up front into the on-Pi scaffold. It MUST record `status:"preset"` (not `"test-mode"`) in the audit file with the token last four only, and MUST exit non-zero with an actionable error if `PLOW_CHAT_TOKEN` or `PLOW_CHAT_CHAT_UID` is missing.
