...
- It maps a Plow chat to a Hermes session source with `platform='plow_chat'`, `chat_id=<chat.uid>`, and member sender metadata as the user identity. `chat_type` is `'dm'` for the home chat (`PLOW_CHAT_CHAT_UID`) and `'group'` for any other chat.
...
- **Group chats are handled by default.** Adding the agent's Plow line to a thread IS the opt-in — an owner who does that expects their agent to take part, so no configuration is required to make it work. `PLOW_CHAT_GROUP_UIDS` does not switch the layer on; it names groups and grants their members authority (below).
- The adapter MUST open one WebSocket subscription per chat it listens to, and MUST poll `GET /v1/chats` every 60 seconds to discover chats added to this agent's line after startup. Nothing pushes chat creation — a ws ticket is per-chat and there is no webhook — so listing is the only way to learn of one.
- **Reach is default-on; authority is always earned.** Taking part in a thread is not the same as trusting the people in it: a discovered chat is answered under the group policy, but its members get no tool or pairing authority until the chat is named in `PLOW_CHAT_GROUP_UIDS` or the operator has spoken in it. This split is what makes default-on reach safe.
...
- The adapter MAY register a `plow_start_group_message` tool. Because group chats are handled by default, the tool MUST NOT be gated on `PLOW_CHAT_GROUP_UIDS` — doing so would leave it unreachable on a stock install. The tool defaults to `dry_run=true` and MUST refuse to send without an explicit `confirm=true`. It MUST report thread adoption separately from delivery, because a delivered message in a thread nobody is subscribed to is not reachable — and its description MUST NOT promise subscription, only attempt it, so a caller reports the adoption result rather than assuming reach. Each recipient entry MUST carry exactly one address; an entry containing the handle delimiter MUST be rejected, since it would be approved as one recipient and delivered to several.
...
- The helper MUST resolve its target env file as follows, highest precedence first: an explicit `--env-file <path>` (or `PLOW_CHAT_ENV_FILE`) which is written verbatim and does not require a scaffold/data dir to exist; else `<data-dir>/.env`, where the data dir is an explicit `--data-dir <dir>`, else `--profile <name>` (or `PLOW_CHAT_PROFILE`) resolving to `<scaffold>/data/profiles/<name>`, else the scaffold's `data/`. The resolved `<data-dir>/.env` MUST match the per-profile `.env` files that downstream install verification reads.
...
- When redeem returns HTTP 410, the helper MUST print a human-readable explanation and exit non-zero, never surfacing only the raw `curl: (22) ... 410` error. It MUST distinguish two cases by how soon the 410 arrives: an **immediate** 410 means the server returned an already-expired/deduped activation rather than minting a fresh one, so the helper MUST warn that re-running now returns the SAME stale code and advise waiting several minutes for the line's activation to clear before retrying; a **genuine later** 410 (the code expired after the human had time to text it) MUST print the exact command to re-run for a fresh code.
...
- The phone-bind activation MAY run up front (e.g. in the operator's preflight prepare-script) rather than mid-install. When it does, the helper MAY be given an explicit `--env-file <path>` (or `PLOW_CHAT_ENV_FILE`) so the verified `PLOW_CHAT_*` creds land in that file — typically the openseed inputs file — instead of a scaffold `<data-dir>/.env` that does not yet exist. With `--env-file`, the helper MUST NOT require a scaffold/data dir; it MUST only ensure the target file's parent directory is writable, and the `.activation.json` audit (written beside the env file) MUST be best-effort — a failure to write the audit MUST NOT fail the run once the creds are written.
- Already-obtained creds (from a real prior phone-bind) MAY be placed at install time via `--from-env`, which reads `PLOW_CHAT_TOKEN` and `PLOW_CHAT_CHAT_UID` from the environment (plus optional `PLOW_CHAT_BASE_URL`, default `https://api.plow.co`), writes them to the resolved target — `PLOW_CHAT_HOME_CHANNEL` is set to the chat uid, as on the live and test paths — and skips the phone-bind dance entirely. This is a real, supported operator path — distinct from `--test-mode` — used to place creds the operator obtained up front into the on-Pi scaffold. It MUST record `status:"preset"` (not `"test-mode"`) in the audit file with the token last four only, and MUST exit non-zero with an actionable error if `PLOW_CHAT_TOKEN` or `PLOW_CHAT_CHAT_UID` is missing.
- Phase 4 verification depends on an external human texting the activation code from the target iPhone, which cannot complete in a headless DinD/CI environment. The helper MAY provide a `--test-mode` that, given operator-supplied `--test-chat-uid`/`--test-token` (or `PLOW_CHAT_TEST_CHAT_UID`/`PLOW_CHAT_TEST_TOKEN`), skips the phone-bind dance and writes those values to the profile `.env`. Test mode MUST NOT contact Plow, MUST record `status:"test-mode"` in the audit file, and is for test validation only — not for real operator activation (in contrast to `--from-env`, which is a supported real path).
